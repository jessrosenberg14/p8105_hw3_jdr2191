...
There are `r count(instacart)` observations and `r ncol(instacart)` variables in the instacart dataset. Each row in the dataset represents an item ordered by a particular customer. Example products ordered include `r head(unique(pull(instacart, product_name)), 5)`. Products come from different aisles, such as `r head(unique(pull(instacart, aisle)), 5)` and different departments, including for example, `r head(unique(pull(instacart, department)), 5)`. 
