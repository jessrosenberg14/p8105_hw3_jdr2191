...
There are 50400 observations and 6 variables in the tidied accelerometer
dataset. The dataset includes data over 35 days (5 weeks). The dataset
also indicates the day of the week that the measurements come from and
an activity count for every minute in a 24-hour period. Lastly, there is
a newly added variable day\_class that indicates if the day of the week
...
``` r
accel %>%
  ggplot(aes(x = minute, y = activity_count, group = day)) + 
