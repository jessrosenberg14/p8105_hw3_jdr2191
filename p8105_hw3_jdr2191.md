...
![](p8105_hw3_jdr2191_files/figure-gfm/aisles_plot-1.png)<!-- --> *Among
items that have been ordered more than 10,000 times, the aisle with the
most items ordered is `fresh vegetables` and the aisle with the fewest
items ordered is `butter`. The only aisles with more than 50,000 items
ordered are `fresh vegetables`, `fresh fruits`,
